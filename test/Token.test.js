...
contract("Token", ([deployer, receiver, exchange]) => {
  ...
  describe("Sending tokens", () => {
    ...
    describe("failure", async () => {
      it("rejects insufficients balances", async () => {
        let invalidAmount;
        invalidAmount = tokens(1000000000); //100 million - greater than supply
        await token
          .transfer(receiver, invalidAmount, { from: deployer })
          .should.be.rejectedWith(EVM_REVERT);

        invalidAmount = tokens(10); //no tokens
        await token
  ...
  describe("Approving tokens", () => {
    ...
    describe('success', () => {
      ...
      it("emits an approval event", async () => {
        ...
        const event = log.args;
        event.owner.toString().should.equal(deployer, "ownser is correct");
  ...
  describe("Delegated token transfers", () => {
    ...
    describe("failure", async () => {
      it("rejects insufficients amounts", async () => {
        const invalidAmount = tokens(1000000000); //too many tokens
        await token
